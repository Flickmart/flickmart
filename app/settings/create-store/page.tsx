...
const page = () => {
  ...
  const [avatar, setAvatar] = useState<string | null | undefined | ArrayBuffer>(
    ""
  );
  return (
    <>
      <header className="shadow-lg py-5 container-px sm:hidden">
      ...
      <main
        className={`px-4 text-center max-w-[500px] mt-[56px] mx-auto ${stage > 2 ? "md:grid md:grid-cols-2 md:fixed md:top-1/2 md:-translate-y-1/2 md:mt-0 md:mx-0 md:left-1/2 md:-translate-x-1/2 md:w-full md:max-w-[900px] md:gap-16 md:items-center" : ""}`}
      >
        <Image
          ...
          height={780}
          alt="create store"
          className={`w-4/5 mx-auto mb-14 md:w-full ${stage >= 3 ? "hidden md:block" : ""}`}
        />
