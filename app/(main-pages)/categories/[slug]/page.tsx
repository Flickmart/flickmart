...
import { Sidebar, SidebarContent, SidebarGroup, SidebarGroupContent, SidebarGroupLabel, SidebarHeader, SidebarTrigger, useSidebar } from "@/components/ui/sidebar";
...
const subCategories = {
    ...
    electronics:["hp", "apple", "lenovo", "acer", "samsung", "xiaomi", "asus", "dell"],
    beauty:["make up", "skin care", "soaps", "hair beauty", "gym equipment"],
    fashion:["bags", "men's clothing", "women's clothing", "men's shoe", "women's shoe", "watches"],
    homes:["single room", "self contain", "flat", "duplex", "bungalow"],
    mobiles:["apple phones", "android phones", "tablet", "phones & tablets accessories"],
                                ...
                                <DropdownMenu>
                                    <DropdownMenuTrigger className="px-5 w-full shadow-md py-2 flex justify-between items-center">
                                        <div className="flex flex-col items-start space-y-1">
                                            <h2 className="font-semibold text-sm capitalize">{location || "select location"}</h2>
                                    ...
                                    <DropdownMenuContent className="w-56">
                                        ...
                                        <DropdownMenuRadioGroup value={location} onValueChange={(value)=> dispatch({type: "location", payload: value})}>
                                            <DropdownMenuRadioItem value="enugu">Enugu</DropdownMenuRadioItem>
                                            <DropdownMenuRadioItem value="nsukka">Nsukka</DropdownMenuRadioItem>
                                ...
                                <div className="w-full text-sm p-5">
                                    ...
                                    <div className="mt-2 flex justify-between items-center">
                                        <div className="w-5/12 flex flex-col items-start">
                                            ...
                                            <Input value={min} type="text" className="w-full border border-flickmart-chat-orange p-2" onChange={
                                                (e)=> dispatch({type: "min", payload: +e.target.value})
                                            } />
                                        </div>
                                        <div className="w-5/12 flex flex-col items-start">
                                            ...
                                            value={max}
                                            onChange={
                                                (e)=> dispatch({type: "max", payload: +e.target.value})
                                            }
                                            />
                                        </div>
                                    </div>
                                    <div className="flex flex-col text-sm mt-6">
                                        <RadioGroup value={priceRange} onValueChange={(value)=> dispatch({type: "priceRange", payload: value})}>
                            ...
                            : displayProducts?.map((product) => {
                                ...
                                <div className="relative" key={product._id}>
                                    <Link href={`/product/${product._id}`}>
                                        <div className="border">
                                            <div className="h-56 w-full overflow-hidden flex justify-center items-center">
                                                {product.images.length && <Image src={product.images[0]} width={500} height={500} alt="category image" />}
                                            </div>
                                            <div className="p-2 mt-1 tracking-tight">
                                                <h2 className="font-semibold">{product.title}</h2>
                                                <span className="text-flickmart-chat-orange font-semibold text-[12px] mt-1">${product.price.toLocaleString()}</span>
                                            </div>
                                    ...
                                    <div 
                                        ...
                                        onClick={async () => {
                                            const saved = await saveProduct({productId: product._id, type: "saved"})
                                            saved?.added? toast.success("Item added to saved") : toast.success("Item removed from saved")
                                        }} 
                                    >
