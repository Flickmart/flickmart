...
console.log(`  - RECOMBEE_PRIVATE_TOKEN: ${recombeePrivateToken.substring(0, 10)}...`);
...
async function defineItemProperties() {
  ...
  try {
    const response = await recombeeClient.send(new requests.Batch(propertyRequests));
    ...
    response.forEach((res: any, index: number) => {
      ...
      if (res.code && res.code !== 200 && res.code !== 201) {
        console.log(`  ❌ ${prop.name}: Error ${res.code} - ${res.message || JSON.stringify(res)}`);
    ...
    if (error.message?.includes("already exists")) {
      ...
      console.error("  ❌ Error defining item properties:", error.message || error);
    }
  }
}
...
async function defineUserProperties() {
  ...
  try {
    ...
    if (error.message?.includes("already exists")) {
      ...
      console.error("  ❌ Error defining user properties:", error.message || error);
    }
  }
}
...
async function syncUsers() {
  ...
  const userUploadRequests = users.map((user) => {
    ...
    return new requests.SetUserValues(user._id, values, { cascadeCreate: true });
  });

  try {
    const response = await recombeeClient.send(new requests.Batch(userUploadRequests));
    ...
    response.forEach((res: any, index: number) => {
      ...
      if (res.code && res.code !== 200 && res.code !== 201) {
        errorCount++;
        console.log(`  ❌ User ${user._id}: Error ${res.code} - ${res.message || JSON.stringify(res)}`);
      } else {
        successCount++;
      }
    });
...
async function syncProducts() {
  ...
  const productUploadRequests = products.map((product) => {
    const values = {
      ...
      timestamp: product.timeStamp ? new Date(product.timeStamp).toISOString() : null,
      price: product.price ?? 0,
      views: product.views ?? 0,
    };
    return new requests.SetItemValues(product._id, values, { cascadeCreate: true });
  });

  try {
    const response = await recombeeClient.send(new requests.Batch(productUploadRequests));
    ...
    response.forEach((res: any, index: number) => {
      ...
      if (res.code && res.code !== 200 && res.code !== 201) {
        errorCount++;
        console.log(`  ❌ Product ${product._id} (${product.title}): Error ${res.code} - ${res.message || JSON.stringify(res)}`);
      } else {
        successCount++;
      }
    });
...
async function main() {
  ...
  try {
    ...
    console.log("  - If properties are still missing, check for type mismatches");
