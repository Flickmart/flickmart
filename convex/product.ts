    ...
    try {
      ...
      const products = await ctx.db
        ...
        .withIndex('by_userId', (q) => {
          if (!user?._id) {
            return q.eq('userId', '' as Id<'users'>); // Handle undefined case with empty string ID
          }
...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const productId = await ctx.db.insert('product', {
      ...
      targetPriceSecond: args.targetPriceSecond,
...
export const likeProduct = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const user = await getCurrentUserOrThrow(ctx);
    if (!user) {
      throw new Error('You need to be logged in to like this product');
    }
...
export const getSimilarProducts = query({
  ...
  handler: async (ctx, args) => {
    ...
    const scoredProducts = allProducts.map((product) => {
      ...
      const _minPrice = Math.min(1, maxPrice); // Avoid division by zero
...
export const getProductsByFilters = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.priceRange) {
      const ranges = {
        cheap: { min: 0, max: 100_000 },
        affordable: { min: 100_000, max: 500_000 },
        moderate: { min: 500_000, max: 1_500_000 },
        expensive: { min: 1_500_000, max: 3_500_000 },
      };
