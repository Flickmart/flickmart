...
export default function TransactionTabs({
  ...
  const activeBackgroundRadius = isMobile ? 'rounded-full' : 'rounded-full px-5 py-2';
  ...
  return (
    <div className={`mb-${isMobile ? '4' : '6'}`}>
      <div className={`flex ${isMobile ? 'gap-6' : 'items-center justify-around'}`}>
        {tabs.map((tab) => (
          <button
            key={tab.id}
          ...
          >
            {activeTab === tab.id && (
              <div className={`${activeBackgroundPadding} absolute inset-0 ${activeBackgroundRadius} bg-orange-100`} />
            )}
